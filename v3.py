...
def normalize_lon(lon: Optional[float]) -> Optional[float]:
    """Wrap longitude to [-180, 180]."""
    if lon is None:
        return None
    return ((lon + 180.0) % 360.0) - 180.0

def validate_lat_lon(lat: Optional[float], lon: Optional[float]) -> Tuple[bool, str]:
    if lat is None or lon is None:
        return False, "Latitude/Longitude missing."
    if not (-90.0 <= lat <= 90.0):
        return False, f"Latitude {lat:.6f} is outside [-90, 90]."
    if not (-180.0 <= lon <= 180.0):
        return False, f"Longitude {lon:.6f} is outside [-180, 180]."
    return True, ""
                    ...
                    [
                        dl.Map(
                            ...
                            children=[
                                ...
                                dl.Marker(
                                    ...
                                    position=[0.0, 0.0],
                                    draggable=True,
                                    children=[dl.Tooltip("Click the map or drag the marker")],
                                ),
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(id="probe-debug", style={"fontSize": "12px", "opacity": 0.8}),
                                html.Div(
                                    [
...
# Overlay catalog callback (search overlays in current view)
@app.callback(
...
# Tiny debug readout for the selected probe point
@app.callback(
    Output("probe-debug", "children"),
    Input("probe-store", "data"),
)
def probe_debug_readout(probe):
    if not probe or "lat" not in probe or "lon" not in probe:
        raise PreventUpdate
    elev = probe.get("elevation")
    elev_txt = "?" if elev is None else f"{elev:.0f} m"
    return f"Selected: lat={probe['lat']:.6f}, lon={probe['lon']:.6f}, elev≈{elev_txt}"
...
@app.callback(
    ...
    Input("mars-map", "click_lat_lng"),
    Input("probe-marker", "position"),
...
def update_probe_location(
    click_lat_lng: Optional[Sequence[float]],
    marker_position: Optional[Sequence[float]],
    ...
    def _commit(lat: Optional[float], lon: Optional[float], msg: str, msg_color: str = "#34d399"):
        # Normalize and validate
        if lon is not None:
            lon = normalize_lon(lon)
        ok, why = validate_lat_lon(lat, lon)
        if not ok:
            status = html.Span(why, style={"color": "#f87171"})
            return dash.no_update, dash.no_update, dash.no_update, dash.no_update, status
        # Fetch elevation and update all sinks
        elevation = fetch_mola_elevation(float(lat), float(lon))  # type: ignore[arg-type]
        probe = {"lat": float(lat), "lon": float(lon), "elevation": elevation}  # type: ignore[arg-type]
        status = html.Span(msg, style={"color": msg_color})
        return [float(lat), float(lon)], probe, float(lat), float(lon), status  # type: ignore[arg-type]

    if trigger == "mars-map" and click_lat_lng:
        # dash-leaflet click is [lat, lon] — keep this order
        try:
            lat = float(click_lat_lng[0]); lon = float(click_lat_lng[1])
        except (TypeError, ValueError):
            raise PreventUpdate
        return _commit(lat, lon, "Point selected from map. You can drag the marker or edit the numbers.", "#60a5fa")

    if trigger == "probe-marker" and marker_position and len(marker_position) == 2:
        lat, lon = float(marker_position[0]), float(marker_position[1])
        return _commit(lat, lon, "Marker dropped. Load terrain to sample this spot.")

    if trigger == "terrain-place-btn" and place_clicks:
        lat, lon = safe_float(lat_value), safe_float(lon_value)
        return _commit(lat, lon, "Marker placed using manual coordinates.", "#60a5fa")
...
# Keep the feature form lat/lon in sync with the currently selected probe point
@app.callback(
    Output("feature-lat", "value"),
    Output("feature-lon", "value"),
    Input("probe-store", "data"),
)
def sync_feature_coords(probe):
    if not probe or "lat" not in probe or "lon" not in probe:
        raise PreventUpdate
    return float(probe["lat"]), float(probe["lon"])
